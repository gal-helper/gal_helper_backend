...
class RecursiveRetriever:
    ...
    async def _recursive_retrieve(
        ...
        if self._total_queries >= self.config.max_query_attempts:
            self.logger.warning(
                f"⚠️ 达到最大查询次数限制 ({self.config.max_query_attempts})，停止递归"
            )
        ...
        if self._total_documents >= self.config.max_total_documents:
            self.logger.warning(
                f"⚠️ 达到最大文档数量限制 ({self.config.max_total_documents})，停止递归"
            )
            return [], {"depth": depth, "query": query, "status": "max_documents_reached"}
        
        # 🛑 保护机制 3：检查重复查询（避免无限循环）
        query_hash = hash(query)
        if query_hash in self._attempted_queries:
            self.logger.debug(f"⚠️ 查询已尝试过，跳过: {query[:50]}...")
        ...
        should_recurse = (
            ...
            and self._total_queries < self.config.max_query_attempts  # 检查查询限制
            and self._total_documents < self.config.max_total_documents  # 检查文档限制
        )
        ...
        if should_recurse:
            ...
            for sub_query in sub_questions:
                ...
                if (self._total_queries >= self.config.max_query_attempts or
                    ...
                    self.logger.info("达到资源限制，停止生成更多子问题")
                    break
...
"""
    ...
    def _calculate_max_depth(self, results: List[RetrievalResult]) -> int:
        ...
        if not results:
            return 1
        # 🛡️ Bug Fix #4：处理 retrieval_depth 为 None 的情况
